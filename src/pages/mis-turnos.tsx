...
export function MisTurnosPage() {
  const { turnos, isPending } = useMisTurnos()

  let content

  if (isPending) {
    content = <LoadingState message="Cargando tus turnos..." />
  } else if (turnos.length === 0) {
    content = <EmptyState message="No tenés turnos reservados" />
  } else {
  ...
  return (
    <section className="flex flex-col gap-6">
      <header>
        ...
        <p className="text-muted-foreground text-sm">Tus reservas activas</p>
      </header>
